...
#pragma pack (pop, n)		// Restore alignment.
		...
		* height = imageHeader.height;
		...
		for (int x0 = 0; x0 < (* height); x0++) {
			for (int y0 = 0; y0 < (* width); y0++) {
				...
				// We will need to use texture swizzle functionality.
	...
	char * ImportTGA_S (const char * fileName, int * width, int * height, int * imageChannels) {
		...
		* height = imageHeader.height;
		...
		for (int x0 = 0; x0 < (* height); x0++) {
			for (int y0 = 0; y0 < (* width); y0++) {
				...
				// Image is stored in BGRA format. Swap B with R.
				imageData[idx++] = buffer[k + 2];
				imageData[idx++] = buffer[k + 1];
				imageData[idx++] = buffer[k];
				if (* imageChannels == 4) {
